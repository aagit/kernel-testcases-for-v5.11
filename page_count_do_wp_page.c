 ...
 *  09854ba94c6aad7886996bfbee2530b3d8a7f4f4. Looks like this has to
 *  be fixed in a forked VM because of:
 *  https://lkml.kernel.org/r/CAHk-=wjZTMsv0_GOyQpLRk_5U1r5W8e21f8sV0jykK=z47hjGQ@mail.gmail.com
 *
 *  The approach of skipping wrprotection on GUP pinned pages, as
 *  described in the above email, is not applicable to mprotect()
 *  concurrent with an O_DIRECT write(), because such case has a
 *  deterministic result no matter if the write() is using O_DIRECT or
 *  buffered I/O. In addition there are false positives possible in
 *  the check if the page is GUP pinned.
 *
